...
if (WRITE) {
  const res = await pool(seed.pages, async (p) => {
    const pageData = { title: p.title, handle: p.handle, body_html: p.body_html, published: true };
    if (p.template_suffix) pageData.template_suffix = p.template_suffix;
    await api('POST', 'pages.json', { page: pageData });
  });
  created.pages = res.ok;
  const withTemplate = seed.pages.filter((p) => p.template_suffix).length;
  console.log(`  ${res.ok} created, ${res.fail} failed (${withTemplate} with custom templates)`);
} else created.pages = seed.pages.length;

// --- COLLECTS (assign products to custom collections) ---
// Custom collections don't auto-populate like smart collections — we must
// explicitly create a "collect" (product→collection membership) for each match.
// seed.json includes a "collects" array mapping product handles to collection handles.
const collectsToCreate = seed.collects ?? [];
if (collectsToCreate.length > 0) {
  console.log(`\n=== Collects — product→collection assignments (${collectsToCreate.length}) ===`);
  if (WRITE) {
    // Build lookup maps: handle → Shopify ID
    console.log('  Fetching product and collection IDs...');
    const existingProducts = await paginate('products.json?fields=id,handle');
    const existingCustom = await paginate('custom_collections.json?fields=id,handle');
    const existingSmart = await paginate('smart_collections.json?fields=id,handle');
    const productIdByHandle = new Map(existingProducts.map((p) => [p.handle, p.id]));
    const collectionIdByHandle = new Map([
      ...existingCustom.map((c) => [c.handle, c.id]),
      ...existingSmart.map((c) => [c.handle, c.id]),
    ]);

    // Filter to only collects we can actually create
    const valid = collectsToCreate.filter((c) => {
      return productIdByHandle.has(c.product_handle) && collectionIdByHandle.has(c.collection_handle);
    });
    console.log(`  ${valid.length} valid of ${collectsToCreate.length} (${collectsToCreate.length - valid.length} skipped — product or collection not found)`);

    const res = await pool(valid, async (c) => {
      await api('POST', 'collects.json', {
        collect: {
          product_id: productIdByHandle.get(c.product_handle),
          collection_id: collectionIdByHandle.get(c.collection_handle),
        },
      });
    });
    console.log(`  ${res.ok} assigned, ${res.fail} failed`);
  } else {
    console.log(`  (dry-run) ${collectsToCreate.length} product→collection assignments`);
  }
}
